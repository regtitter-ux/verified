...
const REGISTRY = [
    ...
    { cat: 'Селф-боты (резерв)', key: 'USER_TOKEN', label: 'Токены аккаунтов (по одному в строке или через запятую)', type: 'multiline', secret: true, live: true,
        help: 'Резервная проверка заходов через личные аккаунты для серверов без бота. Можно несколько токенов — боты подхватят серверы всех аккаунтов. Невидимо для покупателей. Автоматизация аккаунта нарушает ToS Discord — на свой риск.' },
    { cat: 'Селф-боты (резерв)', key: 'RESERVE_GATEWAY', label: 'Gateway-режим (1 = вкл, 0 = только REST)', type: 'text',
        help: 'Пусто/1 — держать постоянное соединение аккаунта (надёжная проверка членства + мгновенный клофбэк). 0 — только REST-запросы (легче, но проверка членства может не работать). Применяется после перезапуска. Выше риск бана в gateway-режиме.' },
